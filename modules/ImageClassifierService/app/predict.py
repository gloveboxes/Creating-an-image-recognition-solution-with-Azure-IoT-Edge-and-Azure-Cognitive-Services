...
def extract_and_resize(img, targetSize):
    ...
    for y in range(targetSize[0]):
        for x in range(targetSize[1]):
            resize_image[y, x] = extract_bilinear_pixel(
...
def extract_and_resize_to_256_square(image):
    h, w = image.shape[:2]
    log_msg("crop_center: " + str(w) + "x" + str(h) +
            " and resize to " + str(256) + "x" + str(256))
...
def resize_down_to_1600_max_dim(image):
    ...
    log_msg("resize: " + str(w) + "x" + str(h) + " to " +
...
def predict_image(image):
    ...
    try:
        ...
        cropped_image = convert_to_nparray(image)

        # Crop the center square and resize that square down to 256x256
        # Commented out the extract and resize as very slow on Raspberry Pi and do the operation on the camera side with opencv
        # resized_image = extract_and_resize_to_256_square(image)

        # Crop the center for the specified network_input_Size
        # cropped_image = crop_center(
        #     image, network_input_size, network_input_size)
